...
global {
	...
	string simulation_name <- "placeholder";
	/* reflex save_raster when: cycle = 50 and !is_batch { // This code block is creating an output raster to create 																				
 ...
 */
	//float step <- 0.5 #day; // this is the step, indicating the model is run every 0.5 day
 ...
 * to create agents or grids, We also define the chance of not crossing a road for a agent
  */
	...
	file boarlocs <- file("../includes/Boar_locs_/BoarLocs_2003-2023_2.shp");
	file dens_kernel <- file("../includes/Rescale_Kern3_2.tif");
	file faunapassage_file <- file("../includes/Faunapassages.shp");
	float chance_of_not_crossing_road <- 1.0;
	...
	reflex reset_at_end_of_year when: current_week = 52 {
	    action_done_for_year <- false;
	    good_mast_year <- false; // Reset this year's status
	    bad_mast_year <- false;  // Reset this year's status
	}

	reflex mast_year when: (current_week = 1) and not action_done_for_year {
  ...
  */
  ...
  int index_m;
	init {
		...
		/* this code creates the roads, and also the inverse, which is not actually used in this model, it works but probably not the most elequent way to make the roads */
    ...
    */
		...
		create sounder number: nb_sounder_init {
			...
			my_cell <- one_of(suitability_cell where (each.shape overlaps chosen_boar_loc.shape and (each.grid_value >= 0.01)));
			// Link the suitability cell of the sounder to a kernel cell
			//my_kernel_cell <- first(kernel where (each.shape overlaps my_cell.shape));
			 ...
			 * The size of the group is randomly assigned between a minimum and maximum group size (minimum 5 adults, 4 yearlings and 10 piglets, maximum 15 adults, 13 yearlings and 30 piglets)) 
			 ...
			 */
			create adult_male number: rnd(2,7) {
				location <- myself.location; // set the sounder for this agent
				index_m <- roulette_wheel(age_distribution_prob_adult);
				birth_date <- current_date - (2 #y + (rnd(index_m + 0.6, index_m + 0.9)) #y);
				my_cell <- myself.my_cell;
			}

			// Create initial adult females for this sounder:
			create adult_female number:  rnd(3,8) {
				my_sounder <- myself;
				index_f <- roulette_wheel(age_distribution_prob_adult);
				birth_date <- current_date - (2 #y + (rnd(index_f + 0.6, index_f + 0.9)) #y);
			}

			create male_yearling number:  rnd(2,5) {
				my_sounder <- myself;
				birth_date <- current_date - (rnd(540, 690) #d);
			}

			create female_yearling number: rnd(2,8) {
				my_sounder <- myself;
				birth_date <- current_date - (rnd(540, 690) #d);
			}

			create piglet number: rnd(10,30) {
				my_sounder <- myself;
 ...
 * Things like birth_date are unassigned, and a value is assigned in the specific species
...
species generic_species {
	...
	reflex update_age when: every(1 #d) {
		if (birth_date != nil) {
			age <- (current_date - self.birth_date) / #year; // the /#year makes it so that the days (in which current_date and birth_date are calculated be divided by 'a year in days' so it returns in years)
		}
...
species sounder parent: generic_species {
	...
	reflex update_local_counts when: every(1 #d){
	...
	reflex sounder_mortality when: every(1 #d) {
	...
	species piglet parent: generic_species {
		sounder my_sounder;
 ...
 * refer to the female yearling, thus we use myself, meaning the piglet once again. You can often hover over these to see what the model is refering to
 ...
 The rnd(10) <= 5 simply picks a random number between 0-10 and looks if it is smaller or equal to 5, if true the code gets executed, else the ELSE code gets executed.
	...
	species female_yearling parent: generic_species {
		sounder my_sounder;
		reflex basic_move  {
			}
 ...
 */
		int nb_offspring;
		float preg_interval <- 180.0 update: preg_interval + (step / #d);

		reflex reproduce when: every(0.5 #d) and /* Create reproduction reflex */
		flip(prob_repr_a) and (current_week > 9) and (current_week < 22) and (self.preg_interval > 224) {
			...
			set nb_offspring <- 0;
			set preg_interval <- 0.0;
		}

	}

	/* nothing new happens here */
	species male_yearling parent: generic_species  {
		sounder my_sounder;
reflex basic_move  {
	}
		reflex age when: every(1 #d) {
 ...
 */
	species adult_female parent: generic_species  {
		int nb_offspring;
		reflex basic_move  {
	}
		sounder my_sounder;
	 ...
	 */
	float recently_female_dispersed <- 0.0 update: recently_female_dispersed + (step / #d);

	reflex dispersing_female when: every(1#d) {
	// Ensure current cycle is week 28
		if ((current_week >= 25 and current_week <= 35) and (local_female_yearlings >= 4) and recently_female_dispersed > 50 ) {
			if flip(0.2) {
	...
	float recently_male_dispersed <- 0.0 update: recently_male_dispersed + (step / #d);

	reflex dispersing_male when: every(1#d) {
		if ((current_week >= 25 and current_week <= 30) and (local_male_yearlings >= 2) and recently_male_dispersed > 50 ) {
			if flip(0.2) {
	...
	/*reflex Carrying_capacity when: every(1#d) {
	// Parameters
...
species adult_male parent: generic_species skills: [moving] {
	float size <- 1000.0;
	rgb color <- #yellow;
			...
			if (empty(potential_neighbors )) {
				...
				loop cell over: potential_neighbors  {
					...
					if cell.contains_road = true and not cell.contains_faunapassage {
						if flip(chance_of_not_crossing_road) {
							continue;
						}
...
species dispersing_males parent: generic_species skills: [moving]  {
...
species dispersing_females parent: generic_species skills: [moving] {
	...
	reflex new_sounder when: every(1#d) {
...
reflex join_sounder when: current_week = 51 {
    // Flag to check if a sounder has been found and joined
    bool sounder_found <- false;

    // Check each neighboring cell
    loop each_cell over: my_cell.neighbors3 {
        if (length(sounder inside each_cell) > 0) {
            // Get the sounder from the cell (assuming only one sounder per cell)
            sounder nearby_sounder <- first(sounder inside each_cell);
            
            // Create a female yearling in the nearby sounder
           
            ask nearby_sounder {
            create female_yearling number: 1 {
                location <- nearby_sounder.location;
                my_sounder <- nearby_sounder;
                birth_date <- myself.birth_date;
            }
            }
            
            
            // Flag that a sounder has been found
            sounder_found <- true;

            // Break the loop as the dispersing_female has found a sounder to join
            break;
        }
    }

    // If a sounder has been joined, make the dispersing female die
    if (sounder_found) {
        do die;
    }
}
...
experiment sounder_movement type: batch {
	parameter "Initial sounders:" var: nb_sounder_init min: 1 max: 1000;
	parameter "chance_of_not_crossing_road:" var: chance_of_not_crossing_road min:0.5 max: 1;
	parameter "fauna_on:" var: fauna_on among: [false, true];
	
	//parameter "Chance of crossing the road:" var: chance_crossing_road min: 0.0 max: 1 category: adult_male;
	//parameter "step:" var: step min: 0.125 max:1;
	output {
